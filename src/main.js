...
fetch(API_URL) // assincrono
  ...
  .then(result => {
    ...
    // Como é mesmo que percorremos um array? 'hm
	  //High Order Function: map, filter, forEach, find, reduce
    data.map(element => {
	  ...
	  divCardText.appendChild(propertyType);
	  divCardText.appendChild(propertyName);
	  divCardText.appendChild(propertyPrice);
    });
  });